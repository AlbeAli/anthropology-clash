...
| 2026-09-26 | Sincronizzazione automatica tra copia locale, GitHub e sessioni cloud                                                                                                                                                       | Richiesta dell'autore: il progetto in locale e quello su GitHub devono essere sempre aggiornati. Le sessioni cloud clonano già da GitHub a ogni avvio; per la copia locale un hook di progetto (`.claude/settings.json`, script Node `.claude/hooks/git-sync.mjs`, eseguibile su Linux e Windows senza dipendenze) fa `git fetch` e un pull solo fast-forward all'avvio della sessione, e mai se ci sono modifiche non committate o commit divergenti; a fine turno avvisa se restano modifiche non committate o commit non pushati. Nessun pull o push forzato, nessuna nuova dipendenza |
