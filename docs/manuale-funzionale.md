...
Versione 0.30 — 2026-09-23 — Stato: pubblicato (M1-M5 chiuse; M6 in corso, §16.9)
...
Backlog M6 (consumo, con aggancio contemporaneo): tre scenari, uno per sessione. Scritto il primo il 2026-09-23, scenario_016 «Il gusto classifica chi classifica», sulla pista del gusto come distinzione: Bourdieu 1979, indagine per questionario condotta in Francia nel 1963 e nel 1967-68 su 1.217 persone, domanda 26 sui soggetti fotografabili; citazioni e numeri di pagina verificati con la ricerca interna di Internet Archive sulla traduzione di Richard Nice (Harvard University Press, 1984). Stato delle altre piste alla stessa data: Veblen 1899 verificato sul testo integrale (`theoryofleisurec01vebl`, «conspicuous consumption of valuable goods is a means of reputability to the gentleman of leisure»); Douglas & Isherwood 1979 verificato il 2026-09-22 (p. 73 della ristampa 1982), ma l'item `worldofgoods00doug` risponde oggi con un errore su `/metadata`, `/details` e `/embed` e non esiste altra copia su Internet Archive, quindi la pista «beni come sistema di classificazione» resta sospesa finché l'item non torna raggiungibile; di Appadurai e Kopytoff 1986 non esiste copia in prestito digitale, solo un caricamento non autorizzato, e la pista «biografia sociale degli oggetti» non è verificabile con il metodo di §16.7; Miller 1987 (`materialculturem0000mill`) e Mintz 1985 (`sweetnesspowerpl0000mint`) sono raggiungibili e interrogabili. Vincolo invariato: non ripetere terreno già coperto da 04 (distruzione ostentata), 05 (hau) e 12 (cibo e classificazione).
...
Bibliografia della pagina "Metodo e fonti": un'opera entra solo se un feedback, una definizione o un `deepen` la cita. È generata dai contenuti (`src/engine/bibliography.ts`), non mantenuta a mano: con i sedici scenari validati sono 66 voci, senza doppioni, ciascuna con lo stato di accesso e gli scenari che la citano. Nessuna voce resta con l'etichetta «accesso da verificare».
...
- 16 scenari in `src/content/it/scenarios/`, tutti validati sul testo delle fonti; 5 concetti in `concepts.json` (`consumo` aggiunto il 2026-09-22), ciascuno con almeno uno scenario giocabile.
...
- App in produzione su Vercel, pagina Metodo con 66 voci di bibliografia generate dai contenuti.
...
4. Tre scenari sul consumo, uno per sessione, con la procedura di §16.6 invariata: fonte primaria letta, ogni citazione confrontata sul testo, `deepen` con sole letture verificate. Ciascuno si apre con un `hook` contemporaneo e risale al caso di studio. Scritto `scenario_016` il 2026-09-23; ne restano due, e la scelta delle piste dipende da quali fonti risultano raggiungibili (§13).
